...
One of the modelling languages that can be used in MDE is Coloured Petri Nets
...
This thesis has resulted in a prototype application called \thename{} for
annotating CPN models with code generation pragmatics. The prototype builds on
the ePNK framework, which uses the Eclipse Modelling Framework (EMF) to provide
an extensible platform for manipulating CPN models.
\thename{} is desgned as a plugin for the Eclipse platform. It can import models
created by CPN Tools and lets the user annotate the models with code generation
pragmatics. The prototype has been evaluated by applying it to a set of protocol
CPN models. We show that using an ontology-based approach yields a robust and
expressive environment for classifying code generation pragmatics, which fits
MDE's pattern of model checking and validation.
We give a detailed case study on CPN modelling of the WebSocket protocol, including
