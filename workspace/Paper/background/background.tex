...
\subsection{Overview}
	...
	encountered in each sub-module. The places in the \nodename{Overview} module use the
	colour set definitions provided in \lstref{overview_colset}.
...
\subsection{Client Application}
	...
	simple coloursets are provided in \lstref{standard_vars}, and some of them can
	be seen in use on arcs in \figref{client_app}.
	...
	lists. \lstref{client_app_vars} shows the variables used in the
	...
	The places at the bottom of \figref{client_app}, \nodename{Client Call} and \nodename{Client Event} 
	represent the interface to the WebSocket layer, and are paired with the
	accordingly named places on the \nodename{Overview} module, which are also
	paired with corresponding places in the \nodename{Client WebSocket} module. The tokens in
	...
	\subsubsection{Control Flow}
		...
		transition, it would consume the |URL| token from \nodename{Target URL}, bind the value of that token to the
		...
		\nodename{Active Connection} and \nodename{Send data} is a two-way arc, which works like two
		...
		Finally, looking at the arc from \nodename{Receive Data} to \nodename{Active Connection}, if the
		\nodename{Client Application} receives a |MESSAGE| where the |OPERATOR| is
		|CLOSE|, nothing is put back into the \nodename{Active Connection} place, and
...
\subsection{The Client WebSocket Module}
	...
	\subsubsection{Process Response Module}
		...
		place \nodename{Waiting for HTTP Response}). The transition has a guard
		inscription where the boolean variable |b| is bound to the result from the
		|isResponseValid| function (\lstref{isResponseValid}), which checks if the
	...
	\subsubsection{Wrap and Send Module}
		...
		\nodename{Queue}, wraps and optionally fragments them in the
...
\subsection{The Server WebSocket Module}
	...
	\nodename{Unwrap and Receive} submodules are the same as the ones for the
	\nodename{Client WebSocket} module. To be more precise, the \nodename{Client
	WebSocket} module and \nodename{Server WebSocket} module both have instances of
	...
	\subsubsection{Get Connection Request Module}
		...
		\begin{lstlisting}[label=isRequestValid,caption=isRequestValid,gobble=2,float]
	...
	\subsubsection{Send Connection Response Module}
		...
		a |CONN_REPLY| to the \nodename{Server Websocket} module.
		...
		which was explained in \lstref{isResponseValid}.
